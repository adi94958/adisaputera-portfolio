...
export const EducationCard: React.FC<EducationCardProps> = ({ 
  ...
  return (
    ...
    >
      ...
      <div className="absolute left-1/2 transform -translate-x-1/2 top-6 w-4 h-4 bg-blue-500 rounded-full border-4 border-white dark:border-gray-800 z-10 shadow-lg"></div>
      ...
      <div className={`w-5/12 ${isLeft ? 'pr-8' : 'pl-8'}`}>
          ...
          <div className="space-y-4">
            <div className="flex flex-col space-y-2">
              ...
              <Text variant="caption" color="muted">
                {formatDate(education.start_date)} - {formatDate(education.end_date)} • {duration}
              </Text>
            </div>
        ...
        <div className={`absolute top-8 ${isLeft ? 'right-0 w-8' : 'left-0 w-8'} h-0.5 bg-blue-500/30`}></div>
      </div>
    </motion.div>
  );
};
